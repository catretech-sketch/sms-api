...
public sealed class SportsService(

    SportsRepository repo, ITenantContext tenant, ITenantFeatureSet features, IClock clock) : ISportsService

{
...

    {
        ...
        if (!FeatureGate.Allowed(tenant, features, FeatureCatalog.Operations))
            ...
            return FeatureGate.Locked<SportsSummaryResponse>(FeatureCatalog.Operations);

        return ApiResult<SportsSummaryResponse>.Ok(await repo.SummaryAsync(clock.UtcNow.Year, ct));
...

    {
        ...
        if (!FeatureGate.Allowed(tenant, features, FeatureCatalog.Operations))
            ...
            return FeatureGate.Locked<IReadOnlyList<SportsTeamResponse>>(FeatureCatalog.Operations);

        return ApiResult<IReadOnlyList<SportsTeamResponse>>.Ok(await repo.ListTeamsAsync(ct));
...

    {
        ...
        if (!FeatureGate.Allowed(tenant, features, FeatureCatalog.Operations))
            ...
            return FeatureGate.Locked<SportsTeamResponse>(FeatureCatalog.Operations);
...

    {
        ...
        if (!FeatureGate.Allowed(tenant, features, FeatureCatalog.Operations))
            ...
            return FeatureGate.Locked<IReadOnlyList<SportsEventResponse>>(FeatureCatalog.Operations);

        return ApiResult<IReadOnlyList<SportsEventResponse>>.Ok(await repo.ListEventsAsync(ct));
...

    {
        ...
        if (!FeatureGate.Allowed(tenant, features, FeatureCatalog.Operations))
            ...
            return FeatureGate.Locked<SportsEventResponse>(FeatureCatalog.Operations);
...

    {
        ...
        if (!FeatureGate.Allowed(tenant, features, FeatureCatalog.Operations))
            ...
            return FeatureGate.Locked<IReadOnlyList<SportsMedalResponse>>(FeatureCatalog.Operations);

        return ApiResult<IReadOnlyList<SportsMedalResponse>>.Ok(await repo.ListMedalsAsync(ct));
...

    {
        ...
        if (!FeatureGate.Allowed(tenant, features, FeatureCatalog.Operations))
            ...
            return FeatureGate.Locked<SportsMedalResponse>(FeatureCatalog.Operations);
