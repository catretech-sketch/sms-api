...
public sealed class HostelService(

    HostelRepository repo, ITenantContext tenant, ITenantFeatureSet features) : IHostelService

{
...

    {
        ...
        if (!FeatureGate.Allowed(tenant, features, FeatureCatalog.Operations))
            ...
            return FeatureGate.Locked<HostelSummaryResponse>(FeatureCatalog.Operations);

        return ApiResult<HostelSummaryResponse>.Ok(await repo.SummaryAsync(ct));
...

    {
        ...
        if (!FeatureGate.Allowed(tenant, features, FeatureCatalog.Operations))
            ...
            return FeatureGate.Locked<IReadOnlyList<HostelBlockResponse>>(FeatureCatalog.Operations);

        return ApiResult<IReadOnlyList<HostelBlockResponse>>.Ok(await repo.ListBlocksAsync(ct));
...

    {
        ...
        if (!FeatureGate.Allowed(tenant, features, FeatureCatalog.Operations))
            ...
            return FeatureGate.Locked<HostelBlockResponse>(FeatureCatalog.Operations);
...

    {
        ...
        if (!FeatureGate.Allowed(tenant, features, FeatureCatalog.Operations))
            ...
            return FeatureGate.Locked<IReadOnlyList<HostelRoomResponse>>(FeatureCatalog.Operations);

        return ApiResult<IReadOnlyList<HostelRoomResponse>>.Ok(await repo.ListRoomsAsync(ct));
...

    {
        ...
        if (!FeatureGate.Allowed(tenant, features, FeatureCatalog.Operations))
            ...
            return FeatureGate.Locked<HostelRoomResponse>(FeatureCatalog.Operations);
...

    {
        ...
        if (!FeatureGate.Allowed(tenant, features, FeatureCatalog.Operations))
            ...
            return FeatureGate.Locked<IReadOnlyList<HostelResidentResponse>>(FeatureCatalog.Operations);

        return ApiResult<IReadOnlyList<HostelResidentResponse>>.Ok(await repo.ListResidentsAsync(ct));
...

    {
        ...
        if (!FeatureGate.Allowed(tenant, features, FeatureCatalog.Operations))
            ...
            return FeatureGate.Locked<HostelResidentResponse>(FeatureCatalog.Operations);
