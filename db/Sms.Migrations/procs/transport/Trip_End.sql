...
BEGIN
    SET NOCOUNT ON;
    UPDATE dbo.Trips SET Status = 'ended', EndedAt = SYSUTCDATETIME()
    WHERE Id = @Id AND TenantId = @TenantId AND Status IN ('live', 'arrived');
    ...
    FROM dbo.Trips WHERE Id = @Id AND TenantId = @TenantId;
END
