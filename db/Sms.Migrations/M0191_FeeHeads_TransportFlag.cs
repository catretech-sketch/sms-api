...
// Reintroduces the FeeHeads.IsTransportFeeHead column + stored-proc changes that were already
// applied to the shared dev database by an unmerged branch's M0184 (student-transport-mapping /
// bulk-add-students worktrees) — this branch's own migration history never picked that change up.
// Idempotent: IF NOT EXISTS / CREATE OR ALTER so it's a safe no-op on the already-drifted dev DB
// and a real schema change on any fresh database.
[Migration(191, "Finance: FeeHeads.IsTransportFeeHead flag for Student transport mapping")]
public sealed class M0191_FeeHeads_TransportFlag : Migration
{
